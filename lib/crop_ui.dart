...
class _CropUIState extends State<CropUI> with TickerProviderStateMixin {
  ...
  void _notifyCropChanged() {
    ...
    if (now.difference(_lastCallbackTime).inMilliseconds < _callbackThrottleMs) {
      return;
    }
  ...
  void _updateCropRect(Rect newRect) {
    ...
    final smallerEdge = screenSize.width < screenSize.height ? screenSize.width : screenSize.height;
  ...
  Rect _validateCropRectSafely(Rect rect) {
    ...
    final minSize = _image != null ? 
      (minPixelSize / (screenSize.width < screenSize.height ? screenSize.width : screenSize.height)).clamp(0.2, 0.5) : 
    ...
    if (width <= 0.0 || height <= 0.0 || left < 0.0 || top < 0.0) {
      ...
      return Rect.fromLTWH(0.1, 0.1, 0.8, 0.8);
    }
        ...
        children: [
          ...
          Expanded(
            child: InteractiveViewer(
              minScale: 0.5,
              maxScale: _isAtMinimumSize ? 5.0 : 3.0, // Allow higher zoom when at minimum size
              child: Center(
...
class _CropWidgetState extends State<CropWidget> {
  ...
  Rect _validateCropRect(Rect rect) {
    const minSize = 0.3; // 30% minimum size (will be refined in gesture handling)
...
class CropImagePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (cropRect.top > imageRect.top) {
      overlayRects.add(Rect.fromLTRB(imageRect.left, imageRect.top, imageRect.right, cropRect.top));
    }
    if (cropRect.bottom < imageRect.bottom) {
      overlayRects.add(Rect.fromLTRB(imageRect.left, cropRect.bottom, imageRect.right, imageRect.bottom));
    }
    if (cropRect.left > imageRect.left) {
      overlayRects.add(Rect.fromLTRB(imageRect.left, cropRect.top, cropRect.left, cropRect.bottom));
    }
    if (cropRect.right < imageRect.right) {
      overlayRects.add(Rect.fromLTRB(cropRect.right, cropRect.top, imageRect.right, cropRect.bottom));
    }
...
class _CropHandlesState extends State<CropHandles> {
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    ...
    switch (_dragType) {
      case 'move':
        newNormalizedRect = normalizedCurrentRect.translate(normalizedDeltaX, normalizedDeltaY);
        break;
      case 'topLeft':
    ...
    final minNormalizedWidth = (minPixelSize / widget.imageDisplaySize.width).clamp(0.2, 0.5);
    final minNormalizedHeight = (minPixelSize / widget.imageDisplaySize.height).clamp(0.2, 0.5);
    ...
    final constrainedRect = Rect.fromLTWH(
      ...
      newNormalizedRect.width.clamp(minNormalizedWidth, 1.0 - newNormalizedRect.left.clamp(0.0, 1.0 - minNormalizedWidth)),
      newNormalizedRect.height.clamp(minNormalizedHeight, 1.0 - newNormalizedRect.top.clamp(0.0, 1.0 - minNormalizedHeight)),
    );

    // Apply aspect ratio constraint if needed (in normalized space)
    final finalNormalizedRect = (widget.lockAspectRatio && widget.aspectRatio != null) 
        ? _constrainToAspectRatioNormalized(constrainedRect, widget.aspectRatio!) 
        : constrainedRect;
  ...
  Rect _normalizedToScreen(Rect normalizedRect) {
    return Rect.fromLTWH(
      widget.imageOffset.dx + normalizedRect.left * widget.imageDisplaySize.width,
      widget.imageOffset.dy + normalizedRect.top * widget.imageDisplaySize.height,
    ...
    return RepaintBoundary(
      child: Stack(
        children: [
          ...
          _buildHandle(_currentRect.topLeft, 'topLeft', handleSize, handleColor),
          _buildHandle(_currentRect.topRight.translate(-handleSize, 0), 'topRight', handleSize, handleColor),
          _buildHandle(_currentRect.bottomLeft.translate(0, -handleSize), 'bottomLeft', handleSize, handleColor),
          _buildHandle(_currentRect.bottomRight.translate(-handleSize, -handleSize), 'bottomRight', handleSize, handleColor),
        ],
      ),
    );
  }

  Widget _buildHandle(Offset position, String dragType, double size, Color color) {
    return Positioned(
      left: position.dx,
      top: position.dy,
      child: GestureDetector(
