...
### Added
- **Video compression**: Added `compressVideo()` method to compress video files with customizable quality, resolution, and bitrate settings. Includes predefined quality presets (low, medium, high, veryHigh) and support for custom compression parameters. Features progress callbacks and option to delete original files after compression. Supported on Android (MediaCodec), iOS/macOS (AVFoundation), with web implementation planned for future release.
